...
#glpsol -m ESTD_model_new.mod -d ESTD_data_new.dat -d ESTD_12TD.dat -o ESTD_output_new.out
...
param cost_limit >=0; # [M€/year] maximum system cost allowed
param einv_limit >=0; # [GWh/year] maximum system energy invested allowed
param ep_constr {TECHNOLOGIES} >= 0;
param agro_land_constr {TECHNOLOGIES} >= 0;
param urban_land_constr {TECHNOLOGIES} >= 0;
param hh_constr {TECHNOLOGIES} >= 0;
param ecosys_constr {TECHNOLOGIES} >= 0;
param rsc_constr {TECHNOLOGIES} >= 0;
param rcm_constr {TECHNOLOGIES} >= 0;
param rcm_woCoal_constr {TECHNOLOGIES} >= 0;
param ep_op {RESOURCES} >= 0;	
param agro_land_op {RESOURCES} >= 0;
param urban_land_op {RESOURCES} >= 0;
param hh_op {RESOURCES} >= 0;
param ecosys_op {RESOURCES} >= 0;
param rsc_op {RESOURCES} >= 0;
param rcm_op {RESOURCES} >= 0;
param rcm_woCoal_op {RESOURCES} >= 0;
...
var TotalRCM_woCOAL >= 0; # RCM_woCOAL_tot [km2*year/year]: Total cumulative raw critical material (RCM_woCOAL) of the system without considering coal
var RCM_woCOAL_constr {TECHNOLOGIES} >= 0; #Total RCM_woCOAL of the technologies
var RCM_woCOAL_op {RESOURCES} >= 0; # Total yearly RCM_woCOAL of the RCM_woCOALs
...
# [Eq. ?]	
subject to totalRCM_woCOAL_calc:	
	TotalRCM_woCOAL = sum {j in TECHNOLOGIES} (RCM_woCOAL_constr [j] / lifetime [j]) + sum {i in RESOURCES} RCM_woCOAL_op [i];
	#JUST RESOURCES :          TotalRCM_woCOAL = sum {i in RESOURCES} RCM_woCOAL_op [i];
	#INCLUDING GREY EMISSIONS: TotalRCM_woCOAL = sum {j in TECHNOLOGIES} (RCM_woCOAL_constr [j] / lifetime [j]) + sum {i in RESOURCES} RCM_woCOAL_op [i];
	
# [Eq. ?]	
subject to rcm_woCoal_constr_calc {j in TECHNOLOGIES}:	
	RCM_woCOAL_constr [j] = rcm_woCoal_constr [j] * F [j];
	
# [Eq. ?]	
subject to rcm_woCoal_op_calc {i in RESOURCES}:	
	RCM_woCOAL_op [i] = rcm_woCoal_op [i] * sum {t in PERIODS, h in HOUR_OF_PERIOD [t], td in TYPICAL_DAY_OF_PERIOD [t]} ( F_t [i, h, td] * t_op [h, td] );
...
/*
# To force the use of waste as there will always be waste
subject to waste_quantity: sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t ['WASTE', h, td] * t_op [h, td]) >= 8900;
*/
...
# Can choose between TotalCost, TotalGWP, TotalEinv and Multi_crit_obj
